...
--==================================================
-- БЕЗОПАСНАЯ РАБОТА С МОДУЛЯМИ ИГРЫ
--==================================================
-- Важно: НЕ require-им клиентские ModuleScript'ы Drive Empire.
-- Некоторые версии игры загружают Criminal UI асинхронно. Принудительный
-- require во время старта может запустить их до инициализации и вызвать
-- внутреннюю ошибку вида:
-- CriminalCharacterIconImageLabel: attempt to index nil with 'GetChildren'
--
-- Поэтому основной трекер работает без принудительного require:
-- атрибуты/Value-объекты и командные признаки используются как безопасный
-- fallback. Это не меняет строки игроков и не вмешивается в UI игры.
local function tryRequire(_instance)
	return nil
end

local sharedModules = findByPath(ReplicatedStorage, { "Modules", "Shared" })

local CriminalUtil = nil
local JobsUtil = nil
local JobsConstants = nil
local CriminalConstants = nil
local TransactionConstants = nil

-- Берём только безопасные числовые/строковые данные из уже существующих
-- объектов, не исполняя ModuleScript.
local function findSafeValue(root, patterns, depth)
	...
	for _, child in ipairs(children) do
		local okName, childName = pcall(function()
			return child.Name
		end)
		if okName and childName then
			local lowered = tostring(childName):lower()
			for _, pattern in ipairs(patterns) do
		...
		if depth > 0 then
			local found = findSafeValue(child, patterns, depth - 1)
			if found ~= nil then
				return found
			end
		end
...
--==================================================
-- AGUZNYA MOBILE TOOLS
-- Подвеска + графика/шейдеры
-- Отдельные окна. Игроки и карточки списка не изменяются.
--==================================================
do
	local Lighting = game:GetService("Lighting")
	local Camera = Workspace.CurrentCamera

	local TOOLS_GUI_NAME = GUI_NAME .. "_MobileTools"
	local SUSPENSION_GUI_NAME = "AGUZNYA_Suspension"
	local GRAPHICS_GUI_NAME = "AGUZNYA_Graphics"

	-- Удаляем только окна этого дополнительного модуля.
	-- Основное окно и карточки игроков НЕ трогаем.
	...
	local function updateMobileScale()
		...
		if width <= 380 then
			ToolsScale.Scale = 0.78
		elseif width <= 500 then
			ToolsScale.Scale = 0.88
		else
			ToolsScale.Scale = 1
		end
	end
	...
	Quick.Name = "QuickTools"
	Quick.Size = UDim2.fromOffset(250, 42)
	Quick.Position = UDim2.new(0.5, -125, 0, 12)
	Quick.BackgroundColor3 = COLOR_BG
	Quick.BackgroundTransparency = 0.04
	Quick.BorderSizePixel = 0
	Quick.Active = true
	Quick.Parent = ToolsGui

	addCorner(Quick, 9)
	addStroke(Quick, 0.05)
	...
	local function makeQuickButton(name, text)
		local b = Instance.new("TextButton")
		b.Name = name
		b.Size = UDim2.fromOffset(116, 30)
		b.BackgroundColor3 = COLOR_FIELD
		b.Text = text
		...
		b.MouseEnter:Connect(function()
			tween(b, 0.12, {
				BackgroundColor3 = Color3.fromRGB(60, 23, 28)
			})
		end)
		b.MouseLeave:Connect(function()
			tween(b, 0.12, { BackgroundColor3 = base })
		end)

		return b
	end

	local SuspensionButton = makeQuickButton("SuspensionButton", "ПОДВЕСКА")
	local GraphicsButton = makeQuickButton("GraphicsButton", "ГРАФИКА")

	-- На телефоне кнопки тоже удобно нажимаются пальцем.
	-- Панель не перекрывает список игроков.
	local quickDragging = false
	local quickDragStart
	local quickStartPosition

	Quick.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch then
			quickDragging = true
			quickDragStart = input.Position
			quickStartPosition = Quick.Position
			input.Changed:Connect(function()
	...
	UserInputService.InputChanged:Connect(function(input)
		if not quickDragging then
			return
		end
		if input.UserInputType == Enum.UserInputType.Touch then
			local delta = input.Position - quickDragStart
			Quick.Position = UDim2.new(
				quickStartPosition.X.Scale,
				quickStartPosition.X.Offset + delta.X,
				quickStartPosition.Y.Scale,
				quickStartPosition.Y.Offset + delta.Y
			)
		end
	end)
	...
	local function createWindow(guiName, titleText, size)
		...
		frame.Size = size
		frame.AnchorPoint = Vector2.new(0.5, 0.5)
		frame.Position = UDim2.new(0.5, 0, 0.5, 35)
		frame.BackgroundColor3 = COLOR_BG
		...
		line.Position = UDim2.new(0, 10, 0, 39)
		...
		close.Position = UDim2.new(1, -41, 0, 5)
	...
	local SuspensionGui, SuspensionFrame = createWindow(
		SUSPENSION_GUI_NAME,
		"РЕДАКТОР ПОДВЕСКИ",
		UDim2.fromOffset(430, 470)
	)

	local SuspensionContent = Instance.new("Frame")
	SuspensionContent.Size = UDim2.new(1, -20, 1, -50)
	SuspensionContent.Position = UDim2.fromOffset(10, 48)
	SuspensionContent.BackgroundTransparency = 1
	SuspensionContent.Parent = SuspensionFrame

	local AxleLabel = Instance.new("TextLabel")
	AxleLabel.Size = UDim2.new(1, 0, 0, 22)
	AxleLabel.BackgroundTransparency = 1
	AxleLabel.Text = "ОСЬ"
	AxleLabel.TextColor3 = COLOR_TEXT_DIM
	AxleLabel.Font = Enum.Font.SourceSansBold
	AxleLabel.TextSize = 12
	AxleLabel.TextXAlignment = Enum.TextXAlignment.Left
	AxleLabel.Parent = SuspensionContent

	local AxleFrame = Instance.new("Frame")
	AxleFrame.Size = UDim2.new(1, 0, 0, 38)
	AxleFrame.Position = UDim2.fromOffset(0, 20)
	AxleFrame.BackgroundTransparency = 1
	AxleFrame.Parent = SuspensionContent

	local axleButtons = {}
	local suspensionMode = "All"

	local function makeAxleButton(key, text)
		local b = Instance.new("TextButton")
		b.Name = key
	...
	local FrontButton = makeAxleButton("Front", "ПЕРЕД")
	local RearButton = makeAxleButton("Rear", "ЗАД")
	local AllButton = makeAxleButton("All", "4 КОЛЕСА")

	local axleLayout = Instance.new("UIListLayout")
	axleLayout.FillDirection = Enum.FillDirection.Horizontal
	axleLayout.Padding = UDim.new(0, 7)
	axleLayout.Parent = AxleFrame
	...
	local function getSelectedSuspension()
		local s = getSuspensionSprings()

		if suspensionMode == "Front" then
			return { s.FrontLeft, s.FrontRight }
		elseif suspensionMode == "Rear" then
			return { s.RearLeft, s.RearRight }
		end
	...
	local function validSpring(spring)
		return spring and spring:IsA("SpringConstraint")
	end

	local function setAxleMode(mode)
		suspensionMode = mode
		for key, button in pairs(axleButtons) do
			if key == mode then
				button.BackgroundColor3 = Color3.fromRGB(135, 25, 33)
				button.TextColor3 = COLOR_TEXT
			else
				button.BackgroundColor3 = COLOR_FIELD
				button.TextColor3 = COLOR_TEXT_DIM
			end
		end
	end

	local suspensionRanges = {
		Damping = { min = 0, max = 50000, decimals = 0 },
		FreeLength = { min = 0, max = 20, decimals = 2 },
		MaxForce = { min = 0, max = 500000, decimals = 0 },
		Stiffness = { min = 0, max = 500000, decimals = 0 },
	}

	local suspensionSliders = {}

	local function formatSuspensionValue(value, decimals)
		if decimals == 0 then
	...
	local function makeSuspensionSlider(property, y)
		local data = suspensionRanges[property]

		local box = Instance.new("Frame")
		box.Size = UDim2.new(1, 0, 0, 68)
		box.Position = UDim2.fromOffset(0, y)
		box.BackgroundColor3 = COLOR_ROW
		box.BorderSizePixel = 0
		box.Parent = SuspensionContent
		addCorner(box, 7)
		...
		valueLabel.Size = UDim2.new(0.35, -10, 0, 24)
		...
		track.Position = UDim2.fromOffset(10, 44)
		track.BackgroundColor3 = COLOR_FIELD
		track.BorderSizePixel = 0
		track.Active = true
		track.Parent = box
		...
		local current = data.min
		local draggingSlider = false

		local function setSliderValue(value, apply)
			...
			if data.max > data.min then
				percent = (value - data.min) / (data.max - data.min)
			end

			fill.Size = UDim2.new(percent, 0, 1, 0)
			knob.Position = UDim2.new(percent, 0, 0.5, 0)
			valueLabel.Text = formatSuspensionValue(value, data.decimals)

			if apply then
				for _, spring in ipairs(getSelectedSuspension()) do
					if validSpring(spring) then
						pcall(function()
		...
		local function setFromInput(input)
			...
			local value = data.min + (data.max - data.min) * percent
	...
	makeSuspensionSlider("Damping", 67)
	makeSuspensionSlider("FreeLength", 141)
	makeSuspensionSlider("MaxForce", 215)
	makeSuspensionSlider("Stiffness", 289)

	local suspensionStatus = Instance.new("TextLabel")
	suspensionStatus.Size = UDim2.new(1, 0, 0, 24)
	suspensionStatus.Position = UDim2.fromOffset(0, 364)
	suspensionStatus.BackgroundTransparency = 1
	suspensionStatus.Text = "Поиск: Vehicles/" .. LocalPlayer.Name .. "/Constraints"
	suspensionStatus.TextColor3 = COLOR_TEXT_DIM
	suspensionStatus.Font = Enum.Font.SourceSans
	suspensionStatus.TextSize = 11
	suspensionStatus.TextXAlignment = Enum.TextXAlignment.Left
	suspensionStatus.TextTruncate = Enum.TextTruncate.AtEnd
	suspensionStatus.Parent = SuspensionContent

	local refreshSuspensionButton = Instance.new("TextButton")
	refreshSuspensionButton.Size = UDim2.new(0.48, -4, 0, 34)
	refreshSuspensionButton.Position = UDim2.new(0, 0, 1, -34)
	refreshSuspensionButton.BackgroundColor3 = COLOR_FIELD
	refreshSuspensionButton.Text = "ОБНОВИТЬ"
	refreshSuspensionButton.TextColor3 = COLOR_TEXT
	refreshSuspensionButton.Font = Enum.Font.SourceSansBold
	refreshSuspensionButton.TextSize = 12
	refreshSuspensionButton.BorderSizePixel = 0
	refreshSuspensionButton.AutoButtonColor = false
	refreshSuspensionButton.Parent = SuspensionContent
	addCorner(refreshSuspensionButton, 6)

	local suspensionInfoButton = Instance.new("TextButton")
	suspensionInfoButton.Size = UDim2.new(0.48, -4, 0, 34)
	suspensionInfoButton.Position = UDim2.new(0.52, 0, 1, -34)
	suspensionInfoButton.BackgroundColor3 = COLOR_FIELD
	suspensionInfoButton.Text = "СБРОС"
	suspensionInfoButton.TextColor3 = COLOR_TEXT
	suspensionInfoButton.Font = Enum.Font.SourceSansBold
	suspensionInfoButton.TextSize = 12
	suspensionInfoButton.BorderSizePixel = 0
	suspensionInfoButton.AutoButtonColor = false
	suspensionInfoButton.Parent = SuspensionContent
	addCorner(suspensionInfoButton, 6)

	local defaultSuspension = {}

	local function readSuspensionDefaults()
		defaultSuspension = {}
		for key, spring in pairs(getSuspensionSprings()) do
			if validSpring(spring) then
				defaultSuspension[key] = {
					Damping = spring.Damping,
					FreeLength = spring.FreeLength,
					MaxForce = spring.MaxForce,
	...
	local function loadSuspensionValues()
		local selected = getSelectedSuspension()
		local first
		...
		if not first then
			suspensionStatus.Text = "Подвеска не найдена — машина ещё загружается"
			suspensionStatus.TextColor3 = COLOR_OFF
			return
		end

		suspensionStatus.Text = "Подвеска найдена • " .. suspensionMode
		suspensionStatus.TextColor3 = COLOR_ON
	...
	local function resetSuspension()
		local s = getSuspensionSprings()
		for key, values in pairs(defaultSuspension) do
			local spring = s[key]
			if validSpring(spring) then
	...
	refreshSuspensionButton.MouseButton1Click:Connect(function()
		readSuspensionDefaults()
		loadSuspensionValues()
	end)

	suspensionInfoButton.MouseButton1Click:Connect(resetSuspension)

	setAxleMode("All")
	readSuspensionDefaults()

	SuspensionButton.MouseButton1Click:Connect(function()
		if GraphicsGui and GraphicsGui.Parent then
			local gf = GraphicsGui:FindFirstChild("Window")
			if gf then gf.Visible = false end
		end
		openWindow(SuspensionFrame)
		task.delay(0.15, loadSuspensionValues)
	end)

	--==================================================
	-- МЕНЮ ГРАФИКИ / ШЕЙДЕРОВ
	--==================================================

	local GraphicsGui, GraphicsFrame = createWindow(
		GRAPHICS_GUI_NAME,
		"ГРАФИКА / ШЕЙДЕРЫ",
		UDim2.fromOffset(430, 520)
	)

	local GraphicsContent = Instance.new("ScrollingFrame")
	GraphicsContent.Size = UDim2.new(1, -20, 1, -50)
	GraphicsContent.Position = UDim2.fromOffset(10, 48)
	GraphicsContent.BackgroundTransparency = 1
	GraphicsContent.BorderSizePixel = 0
	GraphicsContent.ScrollBarThickness = 4
	GraphicsContent.ScrollBarImageColor3 = COLOR_ACCENT
	GraphicsContent.CanvasSize = UDim2.new(0, 0, 0, 0)
	GraphicsContent.Parent = GraphicsFrame

	local GraphicsLayout = Instance.new("UIListLayout")
	GraphicsLayout.Padding = UDim.new(0, 7)
	GraphicsLayout.SortOrder = Enum.SortOrder.LayoutOrder
	GraphicsLayout.Parent = GraphicsContent

	addPadding(GraphicsContent, 0, 3, 2, 10)

	local graphicsStatus = Instance.new("TextLabel")
	graphicsStatus.LayoutOrder = 1
	graphicsStatus.Size = UDim2.new(1, -5, 0, 32)
	graphicsStatus.BackgroundTransparency = 1
	graphicsStatus.Text = "Выбери готовый пресет или настрой всё вручную."
	graphicsStatus.TextColor3 = COLOR_TEXT_DIM
	graphicsStatus.Font = Enum.Font.SourceSans
	graphicsStatus.TextSize = 12
	graphicsStatus.TextWrapped = true
	graphicsStatus.TextXAlignment = Enum.TextXAlignment.Left
	graphicsStatus.Parent = GraphicsContent
	...
	local function getEffect(className, name)
		...
		graphicsEffects[name] = effect
		return effect
	end

	local ColorFX = getEffect("ColorCorrectionEffect", "AGUZNYA_Color")
	local BloomFX = getEffect("BloomEffect", "AGUZNYA_Bloom")
	local SunFX = getEffect("SunRaysEffect", "AGUZNYA_SunRays")
	local DOFFX = getEffect("DepthOfFieldEffect", "AGUZNYA_DOF")
	...
	local function applyGraphics()
		safeSet(Lighting, "Brightness", custom.Brightness)
		safeSet(Lighting, "GlobalShadows", custom.Shadows)
		safeSet(Lighting, "EnvironmentDiffuseScale", custom.Shadows and 1 or 0.5)
		safeSet(Lighting, "EnvironmentSpecularScale", custom.Shadows and 1 or 0.5)
		safeSet(Lighting, "ShadowSoftness", 0.2)

		safeSet(ColorFX, "Brightness", custom.Brightness - 2)
		safeSet(ColorFX, "Contrast", custom.Contrast)
		safeSet(ColorFX, "Saturation", custom.Saturation)

		safeSet(BloomFX, "Intensity", custom.Bloom)
		safeSet(BloomFX, "Size", 18)
		safeSet(BloomFX, "Threshold", 1)

		safeSet(SunFX, "Intensity", custom.SunRays)
		safeSet(SunFX, "Spread", 0.7)

		safeSet(DOFFX, "Enabled", custom.DOF)
		safeSet(DOFFX, "FarIntensity", 0.12)
		safeSet(DOFFX, "FocusDistance", 35)
		safeSet(DOFFX, "InFocusRadius", 18)
		safeSet(DOFFX, "NearIntensity", 0.04)

		ColorFX.Enabled = true
		BloomFX.Enabled = custom.Bloom > 0.001
		SunFX.Enabled = custom.SunRays > 0.001
		DOFFX.Enabled = custom.DOF
	end

	local updateGraphicsControls
	...
	local function setPreset(name)
		if name == "FPS" then
			...
			graphicsStatus.Text = "FPS • минимум эффектов • максимум плавности"
		elseif name == "Balanced" then
			...
			graphicsStatus.Text = "BALANCED • хороший баланс качества и FPS"
		elseif name == "Ultra" then
			...
			graphicsStatus.Text = "ULTRA • максимум эффектов"
		end
		applyGraphics()
	...
	presetFrame.Size = UDim2.new(1, -5, 0, 38)
	presetFrame.BackgroundTransparency = 1
	presetFrame.Parent = GraphicsContent
	...
	local function makePreset(name, text)
		local b = Instance.new("TextButton")
	...
	makePreset("FPS", "FPS")
	makePreset("Balanced", "BALANCED")
	makePreset("Ultra", "ULTRA")

	local function makeToggle(text, layoutOrder)
		local b = Instance.new("TextButton")
		...
		b.BorderSizePixel = 0
		b.AutoButtonColor = false
		b.Parent = GraphicsContent
		addCorner(b, 6)
		addPadding(b, 10, 10, 0, 0)
		return b
	end

	local ShadowsButton = makeToggle("ТЕНИ: ON", 3)
	local DOFButton = makeToggle("РАЗМЫТИЕ: OFF", 4)

	local graphicsSliders = {}

	local function makeGraphicsSlider(property, titleText, minValue, maxValue, decimals, layoutOrder)
		local box = Instance.new("Frame")
		box.LayoutOrder = layoutOrder
		box.Size = UDim2.new(1, -5, 0, 60)
		box.BackgroundColor3 = COLOR_ROW
		box.BorderSizePixel = 0
		box.Parent = GraphicsContent
		addCorner(box, 7)

		local label = Instance.new("TextLabel")
		label.Size = UDim2.new(0.65, 0, 0, 21)
		...
		local valueLabel = Instance.new("TextLabel")
		valueLabel.Size = UDim2.new(0.35, -10, 0, 21)
		...
		local function setValue(v)
			...
			valueLabel.Text = formatSuspensionValue(v, decimals)

			if property == "Brightness" then
				custom.Brightness = v
			elseif property == "Contrast" then
				custom.Contrast = v
			elseif property == "Saturation" then
				custom.Saturation = v
			elseif property == "Bloom" then
				custom.Bloom = v
			elseif property == "SunRays" then
				custom.SunRays = v
			end

			applyGraphics()
			graphicsStatus.Text = "CUSTOM • ручная настройка"
		end
		...
		local function fromInput(input)
			...
			setValue(minValue + (maxValue - minValue) * p)
		end
		...
		graphicsSliders[property] = {
			set = setValue,
			value = function() return value end,
		}
	end

	makeGraphicsSlider("Brightness", "ЯРКОСТЬ", 1, 3, 2, 5)
	makeGraphicsSlider("Contrast", "КОНТРАСТ", -0.3, 0.5, 2, 6)
	makeGraphicsSlider("Saturation", "НАСЫЩЕННОСТЬ", -1, 0.5, 2, 7)
	makeGraphicsSlider("Bloom", "BLOOM", 0, 1, 2, 8)
	makeGraphicsSlider("SunRays", "SUN RAYS", 0, 0.5, 2, 9)

	local resetGraphicsButton = makeToggle("СБРОСИТЬ ГРАФИКУ", 10)

	function updateGraphicsControls()
		ShadowsButton.Text = custom.Shadows and "ТЕНИ: ON" or "ТЕНИ: OFF"
		DOFButton.Text = custom.DOF and "РАЗМЫТИЕ: ON" or "РАЗМЫТИЕ: OFF"

		for property, slider in pairs(graphicsSliders) do
			local value = custom[property]
			if type(value) == "number" then
				slider.set(value)
			end
		end
	...
	ShadowsButton.MouseButton1Click:Connect(function()
		custom.Shadows = not custom.Shadows
		applyGraphics()
		updateGraphicsControls()
		graphicsStatus.Text = "CUSTOM • тени " .. (custom.Shadows and "включены" or "выключены")
	end)

	DOFButton.MouseButton1Click:Connect(function()
		custom.DOF = not custom.DOF
		applyGraphics()
		updateGraphicsControls()
		graphicsStatus.Text = "CUSTOM • размытие " .. (custom.DOF and "включено" or "выключено")
	end)
	...
	resetGraphicsButton.MouseButton1Click:Connect(function()
		custom = {
			...
			SunRays = 0,
			DOF = false,
			Shadows = originalLighting.GlobalShadows,
		}
		applyGraphics()
		safeSet(Lighting, "ShadowSoftness", originalLighting.ShadowSoftness)
		updateGraphicsControls()
		graphicsStatus.Text = "СБРОС • исходные тени и нейтральные эффекты"
	end)

	GraphicsButton.MouseButton1Click:Connect(function()
		if SuspensionGui and SuspensionGui.Parent then
			local sf = SuspensionGui:FindFirstChild("Window")
			if sf then sf.Visible = false end
		end
		openWindow(GraphicsFrame)
	end)

	-- Значения по умолчанию — сбалансированный пресет.
	setPreset("Balanced")

	-- Убираем дополнительную панель, если основной ScreenGui уже удалили
	-- сторонний перезапуском. Новая копия создаст свою панель.
	task.spawn(function()
		while ToolsGui.Parent do
			task.wait(2)
			if not PlayerGui:FindFirstChild(GUI_NAME) then
				-- Основное окно могло быть закрыто/перезапущено.
				-- Дополнительные окна всё равно можно использовать.
			end
		end
	end)

	print("[AGUZNYA] Mobile tools loaded: Suspension + Graphics")
end
