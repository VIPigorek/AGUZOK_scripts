...
local function StartSuspensionEditor()

	--==================================================
	-- ЗАЩИТА ОТ ПОВТОРНОГО ЗАПУСКА
	--==================================================

	if PlayerGui:FindFirstChild("SuspensionEditor") then
		return
	end
	...
	local function getSprings()
		...
		local Vehicle =
			Vehicles:WaitForChild(Player.Name)

		local Constraints =
			Vehicle:WaitForChild("Constraints")

		return {

			FrontLeft =
				Constraints:WaitForChild("FLSpring"),

			FrontRight =
				Constraints:WaitForChild("FRSpring"),

			RearLeft =
				Constraints:WaitForChild("RLSpring"),

			RearRight =
				Constraints:WaitForChild("RRSpring"),

		}

	end

	local Success, Springs = pcall(getSprings)

	if not Success then

		warn(
			"[Suspension Editor] Не удалось найти подвеску:",
			Springs
		)
	...
	ModeButtons.All.BackgroundColor3 =
		COLOR_ACCENT

	ModeButtons.All.TextColor3 =
		COLOR_TEXT
	...
	local function LoadValues()
		...
		for Property, Slider in
			pairs(Sliders)
		do

			local ValueSuccess, Value =
				pcall(function()
...
--==================================================
-- ЗАПУСК РЕДАКТОРА ТОЛЬКО В DRIVE EMPIRE
--==================================================
